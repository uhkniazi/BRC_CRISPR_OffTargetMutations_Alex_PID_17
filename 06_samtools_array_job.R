...
q = paste0('select count(File.idSample) as files, Sample.idData, Sample.title, Sample.id as SampleID from File, Sample
           where (Sample.idData = 35 and File.idSample = Sample.id) group by File.idSample')
dfQuery = dbGetQuery(db, q)
dfQuery$title = gsub(" ", "", dfQuery$title, fixed = T)
dfQuery
# get the count of files
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 35) and (File.idSample = Sample.id) and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 35 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   (File.type = "fastq")')
...
temp = lapply(cvQueries, function(x){
  ...
  # sort the file
  s4 = paste0(cvInput, lf[[1]], '_sort.bam')
  # # remove duplicates
  # s5 = paste0(cvInput, lf[[1]], '_q10_sort_rd.bam')
  p1 = paste(in.s1, s2, s4, sep=' ')
  writeLines(p1, oFile.param)
  return(data.frame(idSample=dfFiles$idSample[1], name=c(s2, s4), type=c('original bam', 'sorted bam'), 
...
writeLines("# Parse parameter file to get variables.
           ...
           bamsort=`sed -n ${number}p $paramfile | awk '{print $3}'`
...
# com2 = paste(p1)
# sort the file
p1 = paste('samtools sort -o', '$bamsort', '$bamfile', sep=' ')
com3 = paste(p1)
# # remove duplicates, for paired end reads
# p1 = paste('samtools rmdup', '$bamq10sort', '$bamrd', sep=' ')
# com4 = paste(p1)
# create index
p1 = paste('samtools index', '$bamsort', sep=' ')
com5 = paste(p1)

writeLines(c(com1, com3, com5), oFile)
writeLines('\n\n', oFile)

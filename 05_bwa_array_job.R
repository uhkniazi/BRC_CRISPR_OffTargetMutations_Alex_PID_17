...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 35) and (File.idSample = Sample.id) ')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
                   where (Sample.idData = 35 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) ')
...
cvGeneIndex = '/users/k1625253/brc_scratch/Data/MetaData/GenomeIndex/hg38_bwa/hg38.fa'
...
# module load
writeLines(c('module load bioinformatics/bwa/0.7.15'), oFile)
writeLines('\n\n', oFile)
...
p1 = paste('bwa mem', cvGeneIndex, '$inr1', '$inr2', '>', '$outsam', sep=' ')
com = paste(p1)
